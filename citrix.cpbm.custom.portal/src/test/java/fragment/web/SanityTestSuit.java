...
@ContextConfiguration(inheritLocations = true, locations = {
		"classpath:/applicationContext-testjmsconsumer.xml",
		"classpath:/applicationContext-workflow-customizations.xml",
...
public class SanityTestSuit extends WebTestsBaseWithMockConnectors {
	...
	public void testEnableService() throws Exception {
		...
		List<ProfileAuthority> beforeAuthorityList = opsProfile
				.getAuthorityList();
		...
		String result = connectorController.enableService(service.getUuid(),
				profileDetails, map);
		Assert.assertNotNull(result);
		Assert.assertEquals("success", result);
		opsProfile = profileService.findProfileByName("Ops Admin");
		List<ProfileAuthority> afterAuthorityList = opsProfile
				.getAuthorityList();
	...
	public void testSaveInstanceWithProducts() {
		...
		connectorController.saveInstance(service.getUuid(), "save",
				configProperties, quickProducts, map, request);
		Assert.assertEquals(service_instances_count + 1,
				serviceInstanceDao.count());
	...
	public void testCreateProductWithMultipleUsageTypes() throws Exception {
		...
		String[] usageTypes = { "ALLOCATED_VM", "RUNNING_VM" };
		...
		Product product = new Product("New", "New_Prod", "New_Prod", "",
				getRootUser());
		product.setCode("New_Prod_Code");
		product.setServiceInstance(serviceInstanceDao.find(serviceInstance
				.getId()));
		...
		for (int i = 0; i < usageTypes.length; i++) {
			...
			List<ServiceUsageType> sutlist = serviceInstance.getService()
					.getServiceUsageTypes();
			...
			if (i == 2)
				...
				operator = "COMBINE";
			jsonString = jsonString+"{conversionFactor: 1.00, operator: " + operator
					+ ", usageTypeId: " + serviceUsageType.getId() + "}";
		...
		Product obtainedProduct = productsController.createProduct(form,
				result, map, response, request);
		...
		List<ProductCharge> obtainedProductCharges = productService
				.getProductCharges(obtainedProduct,
		...
		for (ProductCharge pc : obtainedProductCharges) {
			...
			Assert.assertEquals(currencyValueDAO.find(1L),
					pc.getCurrencyValue());
		}
		Map<ServiceUsageType, List<Product>> productUsageMap = productService
		...
		for (int i = 0; i < usageTypes.length; i++) {
			...
			if (prodlist != null && !prodlist.isEmpty()
					&& prodlist.contains(obtainedProduct)) {
				cond = true;
			}
		}
	...
	/**
	 * Description: Test to create Compute Bundles with None, Monthly, Quarterly
	 * and Annual charge frequency
	 * 
	 * @author vinayv
	 */
	@Test
	public void testCreateBundleForDifferentChargeFrequency() throws Exception {
		...
		String[] chargeFrequency = { "NONE", "MONTHLY", "QUARTERLY", "ANNUAL" };
		for (int i = 0; i < chargeFrequency.length; i++) {
			...
			if (i == 1)
				trialEligible = true;
			ProductBundle obtainedBundle = testCreateProductBundle("1",
					resourceType.getId().toString(), chargeFrequency[i],
					chargeFrequency[i] + "Compute", "USD",
					BigDecimal.valueOf(100), createdAt.getTime(),
					compAssociationJson, ResourceConstraint.PER_USER,
					trialEligible);
			Assert.assertNotNull(obtainedBundle);
			Assert.assertEquals(chargeFrequency[i] + "Compute",
					obtainedBundle.getName());
			Assert.assertEquals(resourceType, obtainedBundle.getResourceType());
			Assert.assertEquals(ResourceConstraint.PER_USER,
					obtainedBundle.getBusinessConstraint());
	...
	/**
	 * Description: Test to create a channel and add bundles to the newly
	 * created channel
	 * 
	 * @author vinayv
	 */
	...
	public void testChannelCreationAndAddingBundles() throws JSONException {
		...
		Channel obtainedChannel = channelController.createChannel(
				"SanityChannel", "SanityChannel", "SanityChannel",
		...
		String result = channelController.attachProductBundles(obtainedChannel
				.getId().toString(), selectedProductBundles, map1);
		Assert.assertNotNull(result);
		Assert.assertEquals("success", result);
		String listBundlesResult = channelController.listbundles(
				obtainedChannel.getId().toString(), map1);
		Assert.assertNotNull(listBundlesResult);
		Assert.assertEquals("productbundle.add", listBundlesResult);
		List<ProductBundleRevision> productbundles = (List<ProductBundleRevision>) map1
				.get("productBundles");
		for (ProductBundleRevision productbundleRevision : productbundles) {
			ProductBundle productBundle = productbundleRevision
					.getProductBundle();
	...
	/**
	 * Description: Test to create a Retail account with Workflow (Completing
	 * the activities in the workflow for the tenant to become Active)
	 * 
	 * @author vinayv
	 */
	@Test
	public void testCreateAccountWithWorkflow() throws Exception {
		...
		activityList
				...
				.getTask()
				.setState(
						com.citrix.cpbm.core.workflow.model.Task.State.SUCCESS);
		runWorkflow(workflow);
		workflow = workflowDAO.findByParam(workflowParam);
		Assert.assertEquals(
				com.citrix.cpbm.workflow.model.Workflow.State.COMPLETED,
				workflow.getState());
		activityList = workflow.getWorkflowActivities();
		for (int i = 0; i < activityList.size(); i++) {
			Assert.assertEquals(Status.SUCCESS, activityList.get(i).getStatus());
		}
		Assert.assertEquals(com.vmops.model.Tenant.State.ACTIVE, tenantDAO
				.find(tenantId).getState());
	}
	...
	public void createSubscription() {
		...
		ProductBundle productBundle = bundleService
				.locateProductBundleById("2");
		...
		Subscription createdSubscription = subscriptionService
				.createSubscription(user, productBundle, productBundle
						.getServiceInstanceId().getUuid(), "VirtualMachine",
						false, false, proxySubscription, new HashMap<String,String>());
		...
		Assert.assertEquals(subscriptionCount + 1,
				subscriptionService.getCountByTenant(tenant));
	 ...
	 * @author vinayv
	 */
	private ProductBundle testCreateProductBundle(String serviceInstanceID,
			String resourceTypeID, String chargeType, String BundleName,
			String currencyCode, BigDecimal currencyValue, Date startDate,
			String jsonString, ResourceConstraint businessConstraint,
			boolean trialEligible) throws Exception {
		
		ServiceInstance serviceInstance = serviceInstanceDao
		...
		RateCardCharge rcc = new RateCardCharge(
				currencyValueService.locateBYCurrencyCode(currencyCode),
				catalogDAO.find(1L), currencyValue, "RateCharge",
				getRootUser(), getRootUser(),
				channelService.getCurrentRevision(null));
		rateCardChargeList.add(rcc);
		String chargeTypeName = chargeType;
		RateCard rateCard = new RateCard("Rate",
				chargeRecurrenceFrequencyDAO.findByName(chargeTypeName),
				new Date(), getRootUser(), getRootUser());
		String compAssociationJson = jsonString;

		ProductBundle bundle = new ProductBundle(BundleName, BundleName, "",
				startDate, startDate, getRootUser());
		...
		BindingResult result = validate(form);
		ProductBundle obtainedBundle = bundleController.createProductBundle(
				form, result, map, response);
		Assert.assertNotNull(obtainedBundle);
	 ...
	 * @author vinayv
	 */
	private void runWorkflow(Workflow workflow) throws SecurityException,
			NoSuchMethodException, IllegalArgumentException,
			IllegalAccessException, InvocationTargetException {
		Method runWorkflowMethod = WorkflowEngineImpl.class.getDeclaredMethod(
				"runWorkflow", Workflow.class);
	...
	private void setupWorkflow(){
		...
		Map<String, Activity> map = applicationContext
				.getBeansOfType(Activity.class);

		for (Entry<String, Activity> entry : map.entrySet()) {
			Dictionary<String, String> props = new Hashtable<String, String>();
			props.put("beanName", entry.getKey());
			bundleContext.registerService(Activity.class.getName(),
					entry.getValue(), props);
		}

		customizationResourceService.setWorkflows(this.getClass()
				.getClassLoader().getResourceAsStream("workflowsTest.xml"));
		customizationResourceService.setTransactionWorkflowMap(this.getClass()
				.getClassLoader()
