...
public abstract class AbstractReportController extends AbstractAuthenticatedController {
  ...
  @RequestMapping(value = "/customerRank", method = {
    RequestMethod.GET
  })
  public String customersRank(@RequestParam(value = "month", required = false) String month,
    ...
    String status = getCustomerRankReport(month, year, modelMap, CustomerRankReport.class, Page.REPORTS_CUSTOMER_RANK,
        getSessionLocale(request));
  ...
  public String customersRank(@RequestParam(value = "month", required = false) String month,
    ...
    String status = getCustomerRankReport(customerRankReportForm.getReportMonth(),
  ...
  private String getCustomerRankReport(String month, String year, ModelMap modelMap,
      @SuppressWarnings("unchecked") Class genericReportKlass, Page page, Locale locale) {
    Report report = null;
    HashMap<String, Object> parameters = new HashMap<String, Object>();
    Calendar cal = Calendar.getInstance();
    if (month == null) {
    ...
    GenericReport crr;
    try {
      crr = (GenericReport) ConstructorUtils.invokeConstructor(genericReportKlass, args, parameterTypes);
      report = reportService.generateReport(crr);
  ...
  @RequestMapping(value = "/productUsage", method = {
    RequestMethod.GET
  })
  public String productUsage(@RequestParam(value = "month", required = false) String month,
    ...
    String status = getCustomerRankReport(month, year, modelMap, MonthlyProductUsage.class, Page.REPORTS_PRODUCT_USAGE,
        getSessionLocale(request));
    if (status.equalsIgnoreCase("failure"))
      return "failure";
    CustomerRankReportForm customerRankReportForm = new CustomerRankReportForm();
  ...
  @RequestMapping(value = "/productUsageDashboardView", method = {
    RequestMethod.GET
  })
  public String productUsageDashboardView(@RequestParam(value = "month", required = false) String month,
      @RequestParam(value = "year", required = false) String year, ModelMap modelMap, HttpServletRequest request) {
    logger.debug("###Entering in productUsage(modelMap) method @GET");
    String status = getCustomerRankReport(month, year, modelMap, MonthlyProductUsage.class, Page.REPORTS_PRODUCT_USAGE,
        getSessionLocale(request));
    if (status.equalsIgnoreCase("failure"))
      return "failure";
    logger.debug("###Exiting productUsageDashboardView(modelMap) method @GET");
    return "report.usageByProductDashboardView";
  }
  ...
  @RequestMapping(value = "/productbundleUsage", method = {
    RequestMethod.GET
  })
  public String productBundleUsage(@RequestParam(value = "month", required = false) String month,
    ...
    String status = getCustomerRankReport(month, year, modelMap, MonthlyProductBundleUsage.class,
  ...
  @RequestMapping(value = "/productBundleUsageDashboardView", method = {
    RequestMethod.GET
  })
  public String productBundleUsageDashboardView(@RequestParam(value = "month", required = false) String month,
      @RequestParam(value = "year", required = false) String year, ModelMap modelMap, HttpServletRequest request) {
    logger.debug("###Entering in productBundleUsageDashboardView(modelMap) method @GET");
    String status = getCustomerRankReport(month, year, modelMap, MonthlyProductBundleUsage.class,
        Page.REPORTS_PRODUCT_BUNDLE_USAGE, getSessionLocale(request));
    if (status.equalsIgnoreCase("failure"))
      return "failure";

    logger.debug("###Exiting productBundleUsageDashboardView(modelMap) method @GET");
    return "report.usageByProductBundleDashboardView";
  }
  ...
  public String customReports(@RequestParam(value = "reportId", required = false) String reportId,
      @RequestParam(value = "param", required = false) String param, ModelMap modelMap, HttpServletRequest request)
      throws Exception {
  ...
  @RequestMapping(value = "/generatecustomreports", method = RequestMethod.GET)
  @ResponseBody
  ...
  @RequestMapping(value = "/downloadcustomreport/{filename}", method = RequestMethod.GET)
  @ResponseBody
  ...
  @RequestMapping(value = "/emailcustomreport/{filename}", method = RequestMethod.GET)
  @ResponseBody
  ...
  @ResponseBody
  public void sendEmail(@RequestParam(value = "imageUrl", required = true) String image,
      @RequestParam(value = "title", required = true) String title, ModelMap modelMap, HttpServletRequest request) {
    logger.debug("###Exiting sendEmail method @");
    File imageFile = new File(request.getSession().getServletContext().getRealPath("/reports/" + image));
    reportService.sendReportAsEmail(new String[] {
      getCurrentUser().getEmail()
    }, "Test", null, imageFile); // TODO Could not test this part as this URL is not getting called from UI currently...
    String message = "report.sent";
    String messageArguments = getCurrentUser().getUsername();
    Event alert = new Event(new Date(), message, messageArguments, getCurrentUser(), Source.PORTAL, Scope.USER,
        Category.ACCOUNT, Severity.INFORMATION, true);
