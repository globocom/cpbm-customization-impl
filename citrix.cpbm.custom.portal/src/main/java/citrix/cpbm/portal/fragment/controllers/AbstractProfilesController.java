...
public abstract class AbstractProfilesController extends AbstractAuthenticatedController {
  ...
  public String showProfiles(@RequestParam(value = "profileId", required = false) String profileId, @RequestParam(
    ...
    logger.debug("###Entering in showProfiles(map) method @GET");
    ...
    logger.debug("###Exiting showProfiles(map) method @GET");
  ...
  public String editProfile(ModelMap map, @PathVariable String profileId, @RequestParam("roles") String authorityNames) {
    logger.debug("###Entering in editProfile(map,profileId,authorityNames) method @POST profileid:" + profileId);
    setPage(map, Page.ADMIN_PROFILES);
    try {
    ...
    logger.debug("###Exiting editProfile(map,profileId,authorityNames) method @POST");
    return "profiles.show";
  }

  private List<ProfileForm> getProfileFormList(Collection<Profile> profileList, boolean ops) {
    logger.debug("###Entering in getProfileFormList(profileList) method");
    List<ProfileForm> formList = new ArrayList<ProfileForm>();
    // Type[] types = Authority.getTypes();
    // List<Type> types = Authority.getOpsNonopsTypes(ops);
    ...
    for (Profile profile : profileList) {
      ...
      // List<Authority> authList = profile.getAuthorityList();
    ...
    logger.debug("###Exiting getProfileFormList(profileList) method");
    return formList;
  }
