  ...
  <c:if test="${noChargesSetYet}">
    <div class='widget_grid inline'>
      <div class="infomessage">
        ...
        <spring:message code="ui.product.current.pricing.message" htmlEscape="false" arguments="${displayDate}" />
      </div>
    </div>
  </c:if>
   ...
   <c:choose>
     ...
     <div class='widget_grid inline<c:out  value="${rowClass}" />' id="<c:out value="${productPrice.currencyValue.currencyCode}" />">
        ...
        <div class="widget_grid_description">
         <span id="price" ><div style="float:left;"><c:out value="${productPrice.currencyValue.sign}" /></div><div style="float:left;">
         <fmt:formatNumber pattern="${currencyFormat}"  value="${productPrice.price}" minFractionDigits="${minFractionDigits}" /></div></span>
        </div>
