...
$(document).ready(function() {
  ...
  $("#view_catalog_action").unbind("click").bind("click", function (event) {
    ...
    window.open("/portal/portal/channel/catalog/view_catalog?channelParam="+channelParam+"&revision="+revision+"&revisionDate="+revisionDate+"&dateFormat="+dateFormat, "_blank", "width=1000,height=850,resizable=yes,menubar=no,status=no,scrollbars=yes,toolbar=no,location=no");
             ...
             $thisDialog.dialog('option', 'buttons', {
                 "OK": function () {
                     ...
                     $.ajax( {
                         type: "POST",
                         url: url,
                         data: {"currencyValData": JSON.stringify(currencyVals),
	    						"channelId": channelId,
	    						"bundleId": bundleId},
                         dataType : "json",
                         async:false,
                  ...
                  for(var i=0; i < currencySignArray.length; i++){
                	  ...
                	  if($("li[id^='channel'].selected.channels").find("#channel_currencies").html().length > 0){
                		  $("li[id^='channel'].selected.channels").find("#channel_currencies").html(
                				  $("li[id^='channel'].selected.channels").find("#channel_currencies").html() + ", " + thisCurrArr[0]);
                	  }
                  }
...
function viewCatalogHistory(historyDate, format, showProductHistory){
    ...
    $("#catalog_content").html("");
    var channelId = $("li[id^='channel'].selected.channels").attr('id').substr(7);
    var inputData = {channelId: channelId};
    if (historyDate != null){
      inputData = {channelId: channelId, historyDate: historyDate, dateFormat: format , showProductHistory: showProductHistory };
    }
...
function addChannelDetailsInListView(channel, currencies){
  ...
  $channelListTemplate.find(".widget_info_popover").find('.raw_contents').find("#info_bubble_displayname").find('.raw_contents_value').find("#value").text(channel.name);
  $channelListTemplate.find(".widget_info_popover").find('.raw_contents').find("#info_bubble_code").find('.raw_contents_value').find("#value").text(channel.code);
  $channelListTemplate.find(".widget_info_popover").find('.raw_contents').find("#info_bubble_code").find('.raw_contents_value').find("#value").text(channel.code);
...
function editChannelDetailsInListViiew(channelId, channelName, channelCode){
	 ...
	 $chanelEdited.find(".widget_info_popover").find('.raw_contents').find("#info_bubble_displayname").find('.raw_contents_value').find("#value").text(channelName);
	 $chanelEdited.find(".widget_info_popover").find('.raw_contents').find("#info_bubble_code").find('.raw_contents_value').find("#value").text(channelCode);
	...
	if (isChannelFormValid(channelForm, "")) {
      ...
      channelNameToDisplay += channelName.substring(count)+"<br>";
			...
			$.ajax({
				...
				success : function(channel) {
    	  			editChannelDetailsInListViiew(channelId, $thisDialog.find("#channelName").val(), $thisDialog.find("#channelCode").val());
					$(".j_channelspopup").hide();
					$("#" + nextstep).show();
				},
				error : function(XMLHttpRequest) {
...
var loadingBarHtml = '<div class="infinite_scrollbarbox" id="loading_intimation"><div class="infinite_loading"></div></div>';
var scrollUrlHitMap = {};
