...
$("#productForm").validate( {
  ...
  "serviceInstance": {
    ...
    "product.code" : {
      ...
      xRemote: {
            condition:function() {
            return $("#product_code").parents(".j_productspopup").is(':visible') && $("#product_code").val() != $("#product\\.code").val();
          },      
    ...
    "productItems" :{
        required: function(element) {
          var baseType = productBaseTypeMap[$("input:radio[name=product\\.productType]:checked").val()];
            if ((baseType == TEMPLATE_BASED ||  baseType == ISO_BASED) && !$(".j_includeUserUploadedTemplateGroup").is(':checked')) {
              return requiredField(element);
            }
            return false;
          }
	  ...
	  $.ajax({
	    ...
	    success: function (json) {
	    	...
	    	for(var i=0; i< json.length; i++){
	    		scaleDropdown.append('<option value="'+json[i].conversionFactor+'" id="'+json[i].name+'">'+json[i].name+'</option>');
...
function changeProductType(current){
   ...
   // update confirmation wizard
    ...
    $step5.find("#confirmProductType").find(".j_description").text($("#producttype"+productTypeId).find(".j_description").text());
}
...
function selecteUserUploadedTemplate(current){
    ...
    if(currentState == "checked"){
     ...
     var $offeringtemplate =  $("#confirmoffering").clone();   
       ...
       var selectedProductType = productBaseTypeMap[$("input:radio[name=product\\.productType]:checked").val()];             
       if (selectedProductType == TEMPLATE_BASED) {
         $offeringtemplate.find(".j_subdescription").text($(current).parents("#userdefinedtemplatesdiv").find("#usertemplateslabel").text());
       } else if (selectedProductType == ISO_BASED) {
...
function editProductDetailsInListView(product){
  ...
  $productListTemplate.find(".widget_info_popover").find('.raw_contents').find("#info_bubble_displayname").find('.raw_contents_value').find("#value").text(product.uom);
  $productListTemplate.find(".widget_info_popover").find('.raw_contents').find("#info_bubble_code").find('.raw_contents_value').find("#value").text(product.code);
...
function addProductNext(current) {
  ...
  if(currentstep == "step3" && product_action == "create" ){
    if($("#showingScalesFor").val() != "" && $("#showingScalesFor").val() != undefined && $("#showingScalesFor").val() != null){
...
function i18nProductTypeDisplayName(field){
  switch(field){
    ...
    case "Secondary Storage":
      if(i18n.label.products.type.SECONDARY_STORAGE != null && i18n.label.products.type.SECONDARY_STORAGE != "undefined"){
    ...
    case "Port Forwarding Rule":
      if(i18n.label.products.type.PORT_FORWARDING_RULE != null && i18n.label.products.type.PORT_FORWARDING_RULE != "undefined"){
        field = i18n.label.products.type.PORT_FORWARDING_RULE;
      }
      break;
    case "Load Balancing Policy":
      if(i18n.label.products.type.LOAD_BALANCER_POLICY != null && i18n.label.products.type.LOAD_BALANCER_POLICY != "undefined"){
...
function addDiscriminatorRow(current) {
    ...
    var lastDiscAddedId = $("#addedDiscriminator_" + sourceUsageTypeId).find("div[id^='addedDiscriminatorValues_']:last").attr("id");
    ...
    newDiscriminatorRow.find("#discriminatorNameSpan").text(discriminatorRowTemplate.find("#discriminatorName option:selected").text());
    newDiscriminatorRow.find("#discriminatorNameSpan").attr("value", discriminatorRowTemplate.find("#discriminatorName option:selected").val());
...
function addDiscriminatorRowInEdit(current){
  ...
  var discriminatorId = discriminatorRowTemplate.find("#discriminatorName option:selected").val();
  var lastDiscAddedCount = 1;
  var lastDiscAddedId = $("#addedDiscriminator_" + medRuleId).find("div[id^='addedDiscriminatorValues_']:last").attr("id");
  if( lastDiscAddedId != undefined){
    var spltArray = lastDiscAddedId.split("_");
    lastDiscAddedCount = parseInt(spltArray[spltArray.length-1]) + 1;
  }
  ...
  var discValDict = jsonUsageTypeDiscriminatorMap[sourceUsageTypeId]["discriminators"][discriminatorId]["discriminatorValues"];
...
function addUsageType(current){
  ...
  newUsageTypeDiscriminatorBlock.find("#add_discriminator_link").attr("id","add_discriminator_link_"+usageTypeDiscriminatorId);
  usageTypeHtml = '<li id="usageTypeLeftPanel_' + serviceUsageTypeId 
        + '" class="widget_navigationlist"  onclick="javascript:showUsageTypeDiscriminators(' + "'"
        + serviceUsageTypeId + "'" +');">'
        + '<span class="navicon RUNNING_VM" style="margin-top:4px;"></span><span class="title" serviceUsageTypeId="'
        + serviceUsageTypeId + '">'
        + newUsageType.find("#usagetype").text() + '</span></li>';
  $("#usageTypeDisp").append(usageTypeHtml);

  var discNamesOptionName = [];
  var actonUrl = productsUrl + "listDiscriminators?serviceInstanceUUID="
      + serviceInstanceUUID + "&serviceUsageTypeId=" + serviceUsageTypeId;
...
function populateEntriesInStep2OfProductEdit(){
  ...
  for(key in jsonMediationRuleMap){
    ...
    populateStep3EntriesOfProductEdit(jsonMediationRuleMap[key]["discriminators"], jsonUsageTypeDiscriminatorMap[serviceUsageTypeId]["discriminators"],
...
function populateStep3EntriesOfProductEdit(discsAlreadyAddedDict, discDict, serviceUsageTypeId, usageTypeName, medRuleId){
  // Add left side panel entries
  usageTypeHtml = '<li id="usageTypeLeftPanel_' + medRuleId 
          + '" medRuleId="'+ medRuleId +'" class="widget_navigationlist"  onclick="javascript:showUsageTypeDiscriminatorsInEdit(' + "'"
          + medRuleId+ "'" +');">'
          + '<span class="navicon RUNNING_VM" style="margin-top:4px;"></span><span class="title" serviceUsageTypeId="'
          + serviceUsageTypeId + '">'
          + l10discAndUsageTypeNames[usageTypeName + "-name"] + '</span></li>';
  $("#usageTypeDisp").append(usageTypeHtml);
  ...
  for(key in discsAlreadyAddedDict){
    ...
    addedDiscElement.find("#discriminatorName").text(l10discAndUsageTypeNames[discsAlreadyAddedDict[key]["discriminatorType"] + "-name"]);
  ...
  if(isDiscAlreadyAdded == true){
    ...
    newUsageTypeDiscriminatorBlock.find("#add_discriminator_link").attr("id","add_discriminator_link_"+serviceUsageTypeId);
    var discNamesOptionName = [];
    for(key in discDict){
      discNamesOptionName.push('<option value="', key, '" serviceUsageTypeId="', serviceUsageTypeId, '">',
          l10discAndUsageTypeNames[discDict[key]["name"] + "-name"], '</option>');
...
function listProducts(current){
  ...
  $("#all_product").css({'color': '#000'});
  $("#active_product").css({'color': '#2C8BBC'});
  $("#retire_product").css({'color': '#2C8BBC'});
...
function listBundles(current){
  ...
  $("#all_productbundle").css({'color': '#000'});
  $("#publish_productbundle").css({'color': '#2C8BBC'});
  $("#unpublish_productbundle").css({'color': '#2C8BBC'});
...
var currentFilterBy;
 ...
 function listProductByFilter(current, filter, currentPage, searchPattern){
  ...
  if(filter == 'active'){
    $("#active_product").css({'color': '#000'});
  }
  
  if(filter == 'retire'){
    $("#retire_product").css({'color': '#000'});
  }
  if(filter == null){
    $("#all_product").css({'color': '#000'});   
  }
...
$(current).parents("#prod_bundles_container").find("li").each(function(){
  ...
  $("#all_productbundle").css({'color': '#2C8BBC'})
  $("#publish_productbundle").css({'color': '#2C8BBC'})
  $("#unpublish_productbundle").css({'color': '#2C8BBC'})
  ...
  if(filter == 'publish'){
    $("#publish_productbundle").css({'color': '#000'});
  }
  
  if(filter == 'unpublish'){
    $("#unpublish_productbundle").css({'color': '#000'});
  }
