...
$(".remove_tenant_link").live("click", function(event) {
   $("#dialog_confirmation").text(dictionary.lightboxremovetenant).dialog('option', 'buttons', getConfirmationDialogButtons("removetenant")).dialog("open");
});
    	  ...
    	  if($("#doNotShowVerifyUserDiv").val()=="true"){
            ...
            success : function(data) {
              ...
              tenantCredentialLiCloned.find('#tenantCredentialLabel').text(key);
                          tenantCredentialLiCloned.find("#tenantCredentialLabel").attr("for", 'tenantCredentialLabel_'+key);              
...
$("#allowSecondaryId").live("click", function(event){
  ...
  $('#imgfavicon').change(function(e){
	...
	if(typeof($('#tenant\\.address\\.country').val()) != 'undefined' ){
		if($('#tenant\\.address\\.country').val()=='US' || $('#tenant\\.address\\.country').val() == 'CA' || 
				  $('#tenant\\.address\\.country').val() == 'AU'|| $('#tenant\\.address\\.country').val() == 'IN' || $('#tenant\\.address\\.country').val() == 'JP'){
		      $("#stateInput").hide();
    ...
    if($('#tenant\\.secondaryAddress\\.country').val()=='US' || $('#tenant\\.secondaryAddress\\.country').val() == 'CA' || 
          $('#tenant\\.secondaryAddress\\.country').val() == 'AU'|| $('#tenant\\.secondaryAddress\\.country').val() == 'IN' || $('#tenant\\.secondaryAddress\\.country').val() == 'JP'){
	...
	$("#issueCreditForm").validate( {
	 ...
	 jQuery.validator.addMethod('userlimitcheck', 
	   ...
	   $.ajax({
	      type : "GET",
	      url : url,
	      data:{userlimit: userlimit, tenantid: tenantid},
	      dataType : "text",
	      async: false,
	      success : function(result) {
	        if(result == 'true'){
	          ...
	          $('#tenant\\.maxUsersError').append('<label class="error userlimitlabel" id="userlimitlabel">'+XMLHttpRequest.responseText+'</label>');
	      }
	   });
  ...
  $(".setOwnerForm").submit(function() {return false;});
  ...
  $("#recordDepositForm").submit(function() {return false;});
      ...
      if(userdetails.countryName != null){
        if($('#user\\.address\\.country').val()=='US' || $('#user\\.address\\.country').val() == 'CA' || 
            ...
            if(state_name_to_state_codes[userdetails.countryName+'.'+userdetails.stateName.toLowerCase()]==null){
              ...
              $("#userAddressStateSelect").val(state_name_to_state_codes[userdetails.countryName+'.'+userdetails.stateName.toLowerCase()]);
            }
  ...
  if ($("#accountForm").valid()) {
    ...
    if (currentstep == "step2") {
      ...
      $.ajax({
        ...
        data : {"user.username":$("#user\\.username").val()},
        dataType : "json",
        async:false,
        success : function(jsonResponse) {
          if(jsonResponse == true) {
            $.ajax({
              ...
              data : {"user.email":$("#user\\.email").val()},
              dataType : "json",
              async:false,
              success : function(jsonResponse) {
                if (jsonResponse == true) {
                  ...
                  var firstInput = $("#" + nextstep).find('input[type=text],input[type=password],input[type=radio],input[type=checkbox],textarea,select').filter(':visible:first');
                  if (firstInput != null) {
                      firstInput.focus();
                  }
                }
...
function editTenantListView(tenantUUID, tenantName){
   ...
   $tenantEdited.find(".widget_info_popover").find('.raw_contents').find('.raw_contents_value : first').find("#value").text(tenantName);
...
$.editTenant = function(jsonResponse){
	    ...
	    if (jsonResponse.accountType=='SYSTEM'){
	         $("#grid_row_container").find("#row"+jsonResponse.param).find("#nav_icon").removeClass(className).addClass('navicon accounts system');
	     }else if (jsonResponse.accountType=='RETAIL'){
	       $("#grid_row_container").find("#row"+jsonResponse.param).find("#nav_icon").removeClass(className).addClass('navicon accounts retail');
	          
	     }else if (jsonResponse.accountType=='Corporate'){
	       $("#grid_row_container").find("#row"+jsonResponse.param).find("#nav_icon").removeClass(className).addClass('navicon accounts corporate');
	     }else if (jsonResponse.accountType=='Trial'){
	       
	       $("#grid_row_container").find("#row"+jsonResponse.param).find("#nav_icon").removeClass(className).addClass('navicon accounts trial');
	       
	     }else  if (jsonResponse.accountType!= 'SYSTEM' && jsonResponse.accountType!='RETAIL' && jsonResponse.accountType!='Corporate' && jsonResponse.accountType!='Trial'){
	       $("#grid_row_container").find("#row"+jsonResponse.param).find("#nav_icon").removeClass(className).addClass('navicon accounts default');
...
$(document).ready(function(){
  ...
  $("#edittenant a").click(function(){
    ...
    $("#creditCardTypeDetailsDiv #stateSecondaryInput input").val($("#creditCardTypeDetailsDiv #stateSecondaryInputReadHidden").text().trim());
    ...
    $("#showCreditCardDetailsDiv #billingStateInput input").val($("#showCreditCardDetailsDiv #billingCCStateInputReadHidden").text().trim());
