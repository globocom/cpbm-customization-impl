...
function listAccount(current){
	  $.ajax( {
			...
			url : accountPath,				
			dataType : "html",
			data:{effectiveTenantParam:effectiveTenantParam},
...
function getCurrentPageAccounts(currentPage,size,charRange,pattern){
	...
	var url = accountPath+"?currentPage="+currentPage+"&charRange="+charRange+"&effectiveTenantParam="+effectiveTenantParam;
	if(size !=null && size !=""){
		url = url+"&size="+size;
	}
...
function getCurrentPageUsers(currentPage, size, charRange) {
	...
	var url = userPath + "?currentPage=" + currentPage + "&charRange="
			+ charRange;
	if (size != null && size != "") {
		url = url + "&size=" + size;
	}
...
function searchaccounts(even, current, charRange) {
	var url = accountPath + "/searchlist?charRange=" + charRange + "&pattern="
			+ todb($(current).val());
	$.ajax( {
		type : "GET",
		url : url,
