...
$(document).ready(function() {
	...
	.addMethod(
     ...
     $(".removealert_link").live("click", function (event) {
       $("#dialog_confirmation").text(dictionary.lightboxremovealert).dialog('option', 'buttons', getConfirmationDialogButtons("removealert")).dialog("open");
     });
...
function showAlertHighChart(spend_cap, criteria_percentage){
  ...
  var  series= [
  {
    data: [{name:'budget', color:"#C0DEF5", y:parseFloat(extra_budget)}],
    showInLegend: false

  },
  {
    name: dictionary.alertHighchartSeriesName1,
    data: [{name:dictionary.alertHighchartCriteriaTooltip,color:'#AA4643', y:parseFloat(spend_cap)}],
    legendIndex:1
...
var options = {
  ...
  "toolTipFormatter" : function(){
    ...
    var pointName = typeof(this.point.name)=='undefined'?': ':'<br>'+this.point.name;
      ...
      var name = '<b>'+dictionary.alertHighchartCriteriaToolstart+': '+criteria_percentage+'% ('+currencySymbol+spend_cap+')</b>'
         +  pointName + currencySymbol + this.y +'.<br>'+dictionary.alertHighchartCriteriaTooltipEmail+':<span style="color:#086A87;"> '+user_email+'</span><br>'+dictionary.alertHighchartCriteriaTooltipPhone+': <span style="color:#086A87;">'+user_phone+'</span>';
      return '<div style="width:290px;word-wrap:break-word;white-space: pre-wrap;">'+name+'</div>'; },
      "toolTipStyle": {
        width:300
    },
    "exportEnabled":false,
    "yAxisStackLabelFormatter":function(){return dictionary.alertHighchartYAxisStackLabel;},
	...
	var actionurl = alertsUrl+"alerts/new?tenant="+$("#tenantId").val();		
	  ...
	  $.ajax( {
	      ...
	      if (budget == null || budget == 0.0) {
	        ...
	        dialogButtonsLocalizer($("#dialog_info"), {'OK':window.parent.g_dictionary.dialogOK}); 
